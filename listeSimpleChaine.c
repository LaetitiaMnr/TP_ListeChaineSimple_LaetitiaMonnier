...
Element* creationElement(int x, int y, int z)
{
    ...
    if ((nouveau_element->donnee = (Point *) malloc(sizeof (Point))) == NULL)
        return NULL;

    nouveau_element->donnee->x = x;
    nouveau_element->donnee->y = y;
    nouveau_element->donnee->z = z;
...
{
    Element *nouveau_element = creationElement(x, y, z);
...
int insFinListe (Liste * liste, Element * courant, int x, int y, int z)
{
    Element *nouveau_element = creationElement(x, y, z);
    if (nouveau_element == NULL)
        return -1;
...
int insListe (Liste * liste, int x, int y, int z, int pos)
{
    if (liste->taille < 2)
        return -1;
...
void affiche (Liste * liste)
{
    ...
    while (courant != NULL){
        printf ("%d-%d-%d \n", courant->donnee->x, courant->donnee->y, courant->donnee->z);
        courant = courant->suivant;
    }
}
